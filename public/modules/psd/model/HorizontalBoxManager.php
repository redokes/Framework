 ...
 * @author wes
 */
class Psd_Model_HorizontalBoxManager extends Psd_Model_BoxManager {

	public function process($layers) {
		// sort by y value
		usort($layers, 'Psd_Model_Importer::sortByX');
		...
		for ($i = 0; $i < $this->numBoxes; $i++) {
			...
			if ($box->numLayers > 1) {
				...
				if ($box->layersOverlap()) {
					...
					$boxManager = new Psd_Model_VerticalBoxManager($box);
				}
				$boxManager->manage();
			}
			else {
