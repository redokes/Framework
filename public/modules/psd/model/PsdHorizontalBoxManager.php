 ...
 * @author wes
 */
class Psd_Class_PsdHorizontalBoxManager extends Psd_Class_PsdBoxManager {

	public function process($layers) {
		// sort by y value
		usort($layers, 'Psd_Class_PsdImporter::sortByX');
		...
		for ($i = 0; $i < $numLayers; $i++) {
			...
			for ($j = 0; $j < $this->numBoxes && $foundBox === false; $j++) {
				...
				// check each box items's x2 value to know if this item is positioned after it
				for ($k = 0; $k < $box->numLayers; $k++) {
					$boxItem = $box->layers[$k];
					if ($layer->x >= $boxItem->x && $layer->x < $boxItem->x2) {
						$foundBox = $j;
						break;
					}
				}
	...
	public function generateHtml() {
		...
		for ($i = 0; $i < $this->numBoxes; $i++) {
			...
			$html .= '<div id="'.$this->boxes[$i]->domId.'" class="psdBoxWrap horizontalBoxWrap" _x="'.$this->boxes[$i]->x.'" _y="'.$this->boxes[$i]->y.'"></div>';
		}
		...
		for ($i = 0; $i < $this->numBoxes; $i++) {
			...
			$newStyles = array(
				...
				'float' => 'left',
				'position' => 'relative',
				'margin-top' => ($box->y - $box->parentBox->y) . 'px'
			);
			if ($i) {
				$newStyles['margin-left'] = ($this->boxes[$i]->x - $this->boxes[$i-1]->x2) . 'px';
			}
	...
	public function generateLayerHtml() {
		...
		for ($i = 0; $i < $this->numBoxes; $i++) {
			...
			// create a horizontal layout manager for this box's items
			if ($box->numLayers > 1) {
				...
				if ($box->layersOverlap()) {
					...
					$boxManager = new Psd_Class_PsdVerticalBoxManager($box);
				}
				$boxManager->manage();
			}
			else {
