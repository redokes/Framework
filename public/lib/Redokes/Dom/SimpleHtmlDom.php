...
// -----------------------------------------------------------------------------
class Redokes_Dom_SimpleHtmlDom {
    public $root = null;
    public $nodes = array();
    ...
    protected function prepare($str, $lowercase=true) {
        ...
        $this->root = new Redokes_Dom_SimpleHtmlDomNode($this);
        $this->root->tag = 'root';
    ...
    protected function parse() {
        ...
        // text
        $node = new Redokes_Dom_SimpleHtmlDomNode($this);
        ++$this->cursor;
	...
	public static function xml2assoc($xml) {
		...
		if (is_array($m[1]))
			for ($i = 0;$i < sizeof($m[1]); $i++) $arr[$m[1][$i]] = self::xml2assoc($m[2][$i]);
		else $arr[$m[1]] = self::xml2assoc($m[2]);
		return $arr;
	}
}